...
--  3. Update all records which have account balances of 0.00 and transactions attempted of 0, so that new account balance is 10.00.
...
--  5. Get username and account balance of 3 users with highest balances, sorting from highest to lowest.
SELECT username, account_balance FROM syntax_practice ORDER BY account_balance DESC LIMIT 3;

--  6. Get username and account balance of 3 users with lowest balances, sorting from lowest to highest.
...
--  8. Add a new record.
INSERT INTO syntax_practice VALUES('anthonymaki','minneapolis',27,42,137.00);

--  9. Delete users that reside in Miami or Phoenix and have completed fewer than 5 transactions.
SELECT * FROM syntax_practice WHERE transactions_completed < 5 AND (city = 'phoenix' OR city = 'miami');
