$(function() {
    ...
    var numTeams = 0;
    //intial variable for the number of teams according to size set to 0
    var sizeNum=0;
    ...
    function appendButtons2() {
        for (var i = 1; i < maxNumber; i++) {
            $("#sizeOfTeams").append("<button type='button' class='btn btn-lg btn-default' id='numTeamsChosen'>" + (i + 1) + "</button>");
        }
    }
    ...
    function appendTeams() {
      $("#teamList").empty();
        ...
        for (var i = 0; i < teams.length; i++) {
            ...
            // $("#teamList ul:last").delay(1000).slideDown(6000, function(){
            //
            // });
        }
        $("#teamList ul").each(function(index) {
          $(this).delay(1000 * index).slideDown(2000);
        });
    }
    ...
    $("#sizeOfTeams").on("click", "#numTeamsChosen", function(event) {
        event.preventDefault();
        numTeams = Math.round(studentArray.length/$(this).text());
        if(numTeams>studentArray.length/2){
          numTeams--;
        }
        console.log("You want", numTeams, "randomized teams");
    });
